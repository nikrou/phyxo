...
function get_image_ids_for_tags($tag_ids, $mode='AND', $extra_images_where_sql='', $order_by='', $use_permissions=true) {
    ...
    if ($use_permissions) {
        $query .= ' LEFT JOIN '.IMAGE_CATEGORY_TABLE.' ic ON id=ic.image_id';
    }

    $query .= ' LEFT JOIN '.IMAGE_TAG_TABLE.' it ON id=it.image_id';
...
function get_common_tags($items, $max_tags, $excluded_tag_ids=array()) {
    ...
    $query = 'SELECT id, name, url_name, count(1) AS counter FROM '.TAGS_TABLE.' AS t';
    $query .= ' LEFT JOIN '.IMAGE_TAG_TABLE.' ON tag_id = id';
    $query .= ' WHERE image_id IN ('.implode(',', $items).')';
    if (!empty($excluded_tag_ids)) {
        $query .= ' AND tag_id NOT IN ('.implode(',', $excluded_tag_ids).')';
    }
    $query .= ' GROUP BY t.id';
