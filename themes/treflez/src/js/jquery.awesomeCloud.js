...
(function ($) {
  ...
  $.fn.awesomeCloud = function (settings) {
  ...
  AwesomeCloud.prototype = {
    create: function () {
      ...
      this.settings.gridSize =
        Math.max(this.settings.size.grid, 4) *
        this.settings.options.printMultiplier;
      ...
      this.settings.color.background = this.colorToRGBA(
        this.settings.color.background
      );
      ...
      switch (this.settings.shape) {
        ...
        case 'x':
          this.settings.shape = function (theta) {
            ...
            return (
              1 /
              (Math.cos(thetaDelta) + Math.sin(thetaDelta) - (2 * Math.PI) / 4)
            );
          };
          break;
        case 'triangle':
          this.settings.shape = function (theta) {
            ...
            return (
              1 / (Math.cos(thetaDelta) + Math.sqrt(3) * Math.sin(thetaDelta))
            );
          };
          break;
        ...
        case 'star':
          this.settings.shape = function (theta) {
            ...
            if (
              ...
              return (
                1 /
                (Math.cos((2 * Math.PI) / 10 - thetaDelta) +
                  3.07768 * Math.sin((2 * Math.PI) / 10 - thetaDelta))
              );
            } else {
              return (
                1 / (Math.cos(thetaDelta) + 3.07768 * Math.sin(thetaDelta))
              );
            }
          };
          break;
      ...
      this.settings.range =
        this.settings.weight.highest - this.settings.weight.lowest;
      ...
      if (this.settings.size.factor === parseInt(0, 10)) {
        ...
        for (i = 0; i < this.words.length; i++) {
          fctx.font =
            this.settings.weightFactor(this.words[i][1]) +
            'px ' +
            this.settings.font;
      ...
      this.settings.color.increment = {
        r:
          (this.settings.color.end.r - this.settings.color.start.r) /
          this.settings.range,
        g:
          (this.settings.color.end.g - this.settings.color.start.g) /
          this.settings.range,
        b:
          (this.settings.color.end.b - this.settings.color.start.b) /
          this.settings.range,
        a:
          (this.settings.color.end.a - this.settings.color.start.a) /
          this.settings.range,
      };
      ...
      if (
        ...
        this.bctx.fillStyle = this.colorToRGBA(
    ...
    allDone: function (canvasID) {
      ...
      canvasDOM.addEventListener(
        ...
        function (ev) {
          ...
          $this.match = null;
          for (
            var idx = 0, blocks = $this.linkTable.length;
            idx < blocks;
            idx++
          ) {
            ...
            if (
              x >= thisWord.x &&
              x <= thisWord.x + thisWord.width &&
              y >= thisWord.y &&
              y <= thisWord.y + thisWord.height
            ) {
      ...
      switch (this.settings.options.color) {
        ...
        default:
          if (typeof this.settings.wordColor !== 'function') {
            ...
            output = this.settings.wordColor(
              word,
              weight,
              fontSize,
              radius,
    ...
    getTextHeight: function (font) {
      ...
      var text = $('<span style="font: ' + font + '">Hg</span>'),
        block = $(
          '<div style="display: inline-block; width: 1px; height: 0px;"></div>'
        ),
        div = $('<div></div>'),
        body = $('body'),
        result = {};
