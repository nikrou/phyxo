{block name="menubar-categories"}
	...
	{if isset($U_START_FILTER)}
	    <a href="{$U_START_FILTER}" title="{'display only recently posted photos'|translate}"><i class="fa fa-filter"></i><span class="visually-hidden">{'display only recently posted photos'|translate}</span></a>
	{/if}
	{if isset($U_STOP_FILTER)}
	    <a href="{$U_STOP_FILTER}" title="{'return to the display of all photos'|translate}"><i class="fa fa-filter"></i><span class="visually-hidden">{'return to the display of all photos'|translate}</span></a>
	{/if}
