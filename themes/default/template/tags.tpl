...
{block name="content"}
	...
	<div class="titrePage">
		...
		{if $display_mode == 'cloud'}
		    <li>
			<a href="{$U_LETTERS}" title="{'group by letters'|translate}" class="pwg-state-default pwg-button" rel="nofollow">
		...
		<tr>
			...
			{foreach from=$letters item=letter}
				...
				<table class="tagLetterContent">
					...
					<tr class="tagLine">
					    <td><a {if !$tag.validated}class="pending{if $tag.status==1} added{else} deleted{/if}"{/if} href="{$tag.URL}" title="{$tag.name}">{$tag.name}</a></td>
