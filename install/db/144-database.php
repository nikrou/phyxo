...
if (in_array($conf['dblayer'], ['mysql'])) {
    $query = 'ALTER TABLE ' . App\Repository\BaseRepository::USER_INFOS_TABLE . ' CHANGE activation_key activation_key VARCHAR(255) DEFAULT NULL,';
    $query .= ' ADD COLUMN activation_key_expire DATETIME DEFAULT NULL AFTER activation_key;';
    $conn->db_query($query);
} elseif ($conf['dblayer'] == 'pgsql') {
    $query = 'ALTER TABLE ' . App\Repository\BaseRepository::USER_INFOS_TABLE . ' ALTER COLUMN activation_key TYPE VARCHAR(255),';
    ...
    $query .= 'INSERT INTO ' . $temporary_table . ' SELECT * FROM ' . App\Repository\BaseRepository::USER_INFOS_TABLE . ';';
    $query .= 'DROP TABLE ' . App\Repository\BaseRepository::USER_INFOS_TABLE . ';';
    $query .= 'ALTER TABLE ' . $temporary_table . ' RENAME TO ' . App\Repository\BaseRepository::USER_INFOS_TABLE . ';';
    $query .= 'ALTER TABLE ' . App\Repository\BaseRepository::USER_INFOS_TABLE . ' ADD COLUMN "activation_key_expire" TIMESTAMP default NULL;';
    $query .= 'COMMIT;';
    $conn->db_query($query);
}

// purge current expiration keys
$conn->db_query('UPDATE ' . App\Repository\BaseRepository::USER_INFOS_TABLE . ' SET activation_key = NULL;');
