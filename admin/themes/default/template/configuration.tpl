{combine_script id="configuration" load="footer" path="admin/themes/default/js/configuration.js"}
      ...
      {foreach from=$main.mail_theme_options item=name key=theme}
        <div class="themeBox {if $main.mail_theme==$theme}themeDefault{/if}">
          <label>
            ...
            <div class="themeShot">
              <img src="{$ROOT_URL}admin/themes/default/template/mail/screenshot-{$theme}.png" alt="" width="150"/>
            </div>
          </label>
          <a href="{$ROOT_URL}admin/themes/default/template/mail/screenshot-{$theme}.png">{'Preview'|translate}</a>
        </div>
      {/foreach}
      </div>
    </li>
