...
$(function () {
	...
	if (users_list.length > 0) {
		datatable = users_list.DataTable({
			...
			ajax: {
				url: '../ws?method=pwg.users.getList&display=all',
		...
		$(document).on('click', '#changePassword [type="submit"]', function () {
			...
			$.ajax({
				url: '../ws?method=pwg.users.setInfo',
				type: 'POST',
				data: {
					pwg_token: pwg_token,
					user_id: userId,
		...
		$(document).on('click', '#user-delete', function () {
			...
			$.ajax({
				url: '../ws?method=pwg.users.delete',
				type: 'POST',
				data: {
					user_id: userId,
					pwg_token: pwg_token
