  ...
  if (users_list.length > 0) {
    ...
    datatable.on('click', '.details-control', function () {
      ...
      if (row.child.isShown()) {
        ...
        const nb_image_page_init = nb_image_page_values.indexOf(
          Number(
            $('#user' + formUser.id + ' input[name="nb_image_page"]').val()
          )
        )
        $('#user' + formUser.id + ' .nb_image_page_infos').html(
          getNbImagePageInfoFromIdx(nb_image_page_init)
        )

        const slider_images = new Slider(
          '#user' + formUser.id + ' [name="nb_image_page"]',
          {
            min: 0,
        ...
        const recent_period_init = $(
          '#user' + formUser.id + ' input[name="recent_period"]'
        ).val()
        $('#user' + formUser.id + ' .recent_period_infos').html(
          getRecentPeriodInfoFromIdx(recent_period_init)
        )

        const slider_period = new Slider(
          '#user' + formUser.id + ' [name="recent_period"]',
          {
            min: 0,
    ...
    function prepareFormUser(user) {
      ...
      if (user.last_visit !== undefined) {
        user.lastVisit_string = sprintf(
          phyxo_msg.lastVisit_pattern,
          user.last_visit_string,
          user.last_visit_since
        )
      }
      ...
      $.ajax({
        ...
        success: function (data) {
          $('.alert')
            .addClass('alert-info')
            .addClass('show')
            .append('<p>&#x2714; ' + phyxo_msg.user_password_updated + '</p>')
            .show()
          datatable.ajax.reload()
        },
        ...
        success: function (json) {
          ...
          $('.alert')
            ...
            .append(
              '<p>&#x2714; ' +
                sprintf(
                  phyxo_msg.username_changed_pattern,
                  '<strong>' + data.result.users[0].username + '</strong>'
                ) +
                '</p>'
            )
            .show()
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $('.alert')
            ...
            .append(
              '<p>&#x2714; ' +
                sprintf(phyxo_msg.user_deleted, username) +
                '</p>'
            )
            .show()
        },
    ...
    $(document).on('click', '.user-infos input[type=submit]', function () {
      ...
      fd.append(
        'enabled_high',
        $('#user' + userId + ' input[name="enabled_high"]').is(':checked')
      )
      ...
      fd.append(
        ...
        nb_image_page_values[
          $('#user' + userId + ' input[name="nb_image_page"]').val()
        ]
      )
      ...
      fd.append(
        'recent_period',
        $('#user' + userId + ' input[name="recent_period"]').val()
      )
      fd.append(
        'expand',
        $('#user' + userId + ' input[name="expand"]').is(':checked')
      )
      fd.append(
        'show_nb_comments',
        $('#user' + userId + ' input[name="show_nb_comments"]').is(':checked')
      )
      fd.append(
        'show_nb_hits',
        $('#user' + userId + ' input[name="show_nb_hits"]').is(':checked')
      )
    ...
    $('select[name="selectAction"]').change(function () {
      ...
      const slider_images = new Slider(
        '#action_nb_image_page [name="nb_image_page"]',
        {
          min: 0,
          max: nb_image_page_values.length - 1,
          value: nb_image_page_init,
      ...
      const slider_period = new Slider(
        '#action_recent_period [name="recent_period"]',
        {
          min: 0,
          max: recent_period_values.length - 1,
          value: recent_period_init,
      ...
      switch (action) {
        ...
        case 'show_nb_comments':
          data.show_nb_comments = $(
            'input[name="show_nb_comments"]:checked'
          ).val()
          break
        case 'show_nb_hits':
