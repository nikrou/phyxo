$(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.stat == 'ok') {
                    ...
                    $('#showAddUser .infos')
                        .html(sprintf(newUser_pattern, new_user.username))
                        .show();
                    checkSelection();
    ...
    $('#action_nb_image_page .nb_image_page').slider({
        ...
        stop: function(event, ui) {
            $('#action_nb_image_page input[name=nb_image_page]')
    ...
    $('#action_recent_period .recent_period').slider({
        ...
        stop: function(event, ui) {
            $('#action_recent_period input[name=recent_period]')
    ...
    function fnFormatDetails(oTable, nTr) {
        ...
        $.ajax({
            url: '../ws.php?method=pwg.users.getList',
            type: 'POST',
            data: {
                user_id: userId,
                display: 'all'
            ...
            success: function(data) {
                ...
                if (data.stat == 'ok') {
                    ...
                    user.registeredOn_string = sprintf(registeredOn_pattern, user.registration_date_string, user.registration_date_since);
                    ...
                    var template = _.template($('script.userDetails').html());
                    ...
                    $.each(
                        $.grep(user.groupOptions, function(group) {
                    ...
                    $('#user' + userId + ' .nb_image_page').slider({
                        ...
                        stop: function(event, ui) {
                            $('#user' + userId + ' input[name=nb_image_page]')
                                .val(nb_image_page_values[ui.value])
                                .trigger('change');
                        }
                    });
                    ...
                    $('#user' + userId + ' .recent_period').slider({
                        ...
                        stop: function(event, ui) {
                            $('#user' + userId + ' input[name=recent_period]')
                                .val(recent_period_values[ui.value])
                                .trigger('change');
                        }
                    });
        ...
        return '<div id="user' + userId + '" class="userProperties"><img class="loading" src="./theme/images/ajax-loader-small.gif" alt=""></div>';
    }
    ...
    $(document).on('click', '.changePassword .cancel', function() {
        var userId = $(this)
            .parentsUntil('form')
            .parent()
            .find('input[name=user_id]')
    ...
    $(document).on('click', '.changeUsernameOpen a', function() {
        ...
        $('#user' + userId + ' .changeUsername input[type=text]')
            .val(username)
            .focus();
    ...
    $(document).on('click', '.changeUsername .cancel', function() {
        var userId = $(this)
            .parentsUntil('form')
            .parent()
            .find('input[name=user_id]')
    ...
    var aoColumns = [
        ...
        },
        {
            mRender: function(data, type, full) {
                return (
                    '<label><input type="checkbox" data-user_id="' +
                    full[0] +
                    '"> ' +
                    data +
                    '</label> <a title="' +
                    phyxo_msg.open_user_details +
                    '" class="fa fa-pencil openUserDetails">' +
                    phyxo_msg.edit +
                    '</a>'
                );
            }
    ...
    var oTable = $('#userList').dataTable({
        ...
        oLanguage: {
            ...
            sInfo: phyxo_msg.showing_to_users
                .replace('%s', '_START_')
                .replace('%s', '_END_')
                .replace('%s', '_TOTAL_'),
    ...
    function checkSelection() {
        if (selection.length > 0) {
            ...
            $('#applyOnDetails').text(sprintf(applyOnDetails_pattern, selection.length));

            if (selection.length == allUsers.length) {
                $('#selectedMessage').text(sprintf(selectedMessage_all, allUsers.length));
            } else {
                $('#selectedMessage').text(sprintf(selectedMessage_pattern, selection.length, allUsers.length));
            }
        } else {
            $('#forbidAction').show();
            $('#permitAction').hide();

            $('#selectedMessage').text(sprintf(selectedMessage_none, allUsers.length));
        }
